...
if(!isset($_SESSION['radmin'])){
    if (isset($_REQUEST['rsignin']))
    {
        ...
        $rsubject=mysqli_real_escape_string($conn,trim($_REQUEST['rsubject']));
        $sql="select email,password,subject from adminlogin where email='".$remail."' and password='".$rpassword."' and subject='".$rsubject."' limit 1";
        $result=$conn->query($sql);
        if($result->num_rows==1)
        {
            $_SESSION['radmin']=true;
            $_SESSION['remail'] = $remail;
            $_SESSION['rsubject']=$rsubject;
         ...
         <div class="row justify-content-center mt-5">
             <div class="col-sm-6 col-md-4 text-white " style="background-color: rgba(0, 0, 0, 0.4); border-radius: 10px;">
                 <form action="" class="p-4 shadow-lg" method="POST" id="myForm">
                     ...
                     <div class="form-group">
                        <i class="fas fa-key mr-2"></i><label for="name" class="font-weight-bold">Subject</label>
                        <input type="text" name="rsubject" id="rsubj" placeholder="Enter subject" class="form-control">
                    </div>
