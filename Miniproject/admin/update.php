...
if($_GET['q']=='deletevivaqns')
{
    if(isset($_GET['eid']))
    {
        $eid=$_GET['eid'];
        $sql=mysqli_query($conn,"select * from vivaquestions where eid='$eid'");
        while($row=mysqli_fetch_array($sql))
        // $c=mysqli_query($conn,"delete from history where eid='$eid'");
        $d=mysqli_query($conn,"delete from vivaquestions where eid='$eid'");
        $e=mysqli_query($conn,"delete from viva where eid='$eid'");
        header("location:showviva.php");
    }
}

if($_GET['q']=='vivaadding')
{
    $eid=$_SESSION['eid'];
    $n=$_SESSION['qns'];
    for($i=1;$i<=$n;$i++)
    ...
    $id=uniqid();
    $qns=$_REQUEST['qns'.$i];
    $sql=mysqli_query($conn,"insert into vivaquestions values ('$eid','$id','$qns')");
    }
}
?>